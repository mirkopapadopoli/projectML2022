...
"ROC", trControl = control)
  probs = predict(model, testset, testset[,! names(testset) %in% c("is_safe")], type = "prob")
  ROC = roc(response = testset[,c("is_safe")], predictor = probs$yes,
  levels = levels(testset[,c("is_safe")]))
  plot(ROC,type="S", col=color, add = added)
  ROC
  return(model)
}
```
...
control = trainControl(method = "repeatedcv", number = 10,repeats = 3,
classProbs = TRUE, summaryFunction = twoClassSummary)
svm.modelcm= train(make.names(is_safe) ~ ., data = trainset, method = "svmRadial", metric =
"ROC", trControl = control)
```
