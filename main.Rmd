...
```{r}
generate_roc_single_model(train.tree.model, testset[,!names(testset) %in% c("Prediction","PredictionPR")], 2, "rpart")
```

```{r}
generate_roc_single_model(train.tree.model, testset[,!names(testset) %in% c("Prediction","PredictionPR")], 1, "rpart")
```
...
```{r}
generate_roc_single_model(train.svm.model.prob, testset[,!names(testset) %in% c("Prediction","PredictionPR")], 2, "svm")
```

```{r}
generate_roc_single_model(train.svm.model.prob, testset[,!names(testset) %in% c("Prediction","PredictionPR")], 1, "svm")
```
...
comp_models_roc = function(trainset, meth, control, testset, color, added){
  model= train(make.names(is_safe) ~ ., data = trainset, method = meth,  trControl = control)
  probs = predict(model, testset[,! names(testset) %in% c("is_safe")], type = "prob")
  colnames(probs)[1] ="no"
  colnames(probs)[2] ="yes"
  ROC = roc(response = testset[,c("is_safe")], predictor = probs$no,
