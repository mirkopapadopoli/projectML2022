...
```{r}
allset= split.data(waterquality_pre, s = 300)
...
```{r}
prunedDecisionTree = prune(modeltree, cp=.016)
```
...
confusionmatrix.prunedtree = table(testset$is_safe, testset$PredictionPR)
...
```{r}
svm.model = svm(is_safe ~ ., data=trainset, kernel='linear', cost=1, scale=TRUE) 
```
...
```{r}
svm.table=table(svm.pred, testset$is_safe)
svm.table
```

```{r}
tuned = tune.svm(is_safe ~ ., data = trainset,kernel='linear',
cost=c(10,0.1,1,0.01))
```

```{r}
svm.pred = predict(tuned$best.model, test_svm)
```
