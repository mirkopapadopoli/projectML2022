...
```{r}
svmT.pred = predict(tuned$best.model, test_svm)
```
...
prob.treePR = attr(pred.treePR.prob, "probabilities")
pred.treePR.roc = prob.treePR[, 2] 
```

```{r}
pred.rocr = prediction(pred.treePR.roc, testset$is_safe) 
...
pred.svmT.prob = predict(tuned$best.model, test.tree, probability=TRUE)
...
perf.tpr.rocr.svmT = performance(perf.rocr.svmT, "tpr","fpr") 
